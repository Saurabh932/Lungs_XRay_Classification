...
class ModelTrainer:
    ...
    def train(self, optimizer: Optimizer) -> None:
        ...
        Input: model, devicem train_loader, optimizer, epoch
        ...
        try:
            ...
            for batch_idx, (data, target) in enumerate(pbar):
                ...
                RNN generally not used in CNN or specific requirements prediction on data"""

                y_pred = self.model(data)
                logging.info(f"Shape of y_pred: {y_pred.size()}, Shape of target: {target.size()}")
                print("Shape of y_pred:", y_pred.size())
                print("Shape of target:", target.size())

                # Ensure batch sizes match
                assert data.size(0) == target.size(0), f"Input batch size {data.size(0)} does not match target batch size {target.size(0)}"


                # Calculating the loss given the prediction
                ...
                # Getting the index of the log-propbability corresponding to the max value
